...
class DockerImagePuller:
    ...
    def pull_image(self):
        ...
        token = self.fetch_token(name=self.name)
        manifest = self.fetch_manifest(
            token=token, name=self.name, tag=self.tag, ouput_dir=None
        )
        sub_manifest_json = self.fetch_sub_manifest(
            token,
            self.name,
            manifest,
            output_dir=None,
        ...
        self.export_repositories_file(
            self.name, self.tag, output_dir=self.output_dirpath
        )
        self.export_manifest_file(
            self.name,
            self.tag,
            sub_manifest_json,
            layers=layers,
    ...
    def fetch_token(self, name: str) -> str:
    ...
    def fetch_manifest(
        self, token: str, name: str, tag: str, ouput_dir: str | Path = None
    ) -> json:
        ...
        if ouput_dir is not None:
            with open(
                ouput_dirpath.joinpath("manifest.json"),
                "w",
    ...
    def fetch_sub_manifest(
        ...
        if output_dir is not None:
            with open(
                output_dirpath.joinpath("sub_manifest.json"),
                "w",
                encoding=self.encoding,
            ) as wb:
    ...
    def fetch_config_file(
        self, token, name, sub_manifest_json, output_dir: str | Path = "./"
    ):
        ...
        with open(config_file, "w", encoding=self.encoding) as wb:
            json.dump(config_json, wb, indent=4)
        return config_json

    def fetch_layer_file(
        self, token, name, layer_meta, config_json: json, output_dir: str
    ):
        ...
        output_sub_dirpath.mkdir(exist_ok=True)
        with open(
            output_sub_dirpath.joinpath("json"), "w", encoding=self.encoding
        ) as fout:
    ...
    def export_repositories_file(self, name, tag, output_dir: str):
        ...
        with open(
            output_dirpath.joinpath("repositories"),
            "w",
            encoding=self.encoding,
        ) as fout:
    ...
    def export_manifest_file(
        self, name, tag, sub_manifest_json, layers: list[str], output_dir: str
    ):
        ...
        with open(
            output_dirpath.joinpath("manifest.json"),
            "w",
            encoding=self.encoding,
        ) as fout:
